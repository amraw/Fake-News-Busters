...
import pickle
import csv
from feature_extraction import get_all_features, get_tfidf_vec, headline_body_vec
...
def feed_forward_model(body_length, numb_layers):
    fexc = Preprocessing()
    data = load_data()
    ...
    tfidf_vec = get_tfidf_vec(train_headlines_cl + train_bodies_cl + test_headlines_cl + test_bodies_cl, lim_unigram=5000)

    train_headline_body_vec = headline_body_vec('train', train_headlines_cl, train_bodies_cl, tfidf_vec)
    test_headline_body_vec = headline_body_vec('test', test_headlines_cl, test_bodies_cl, tfidf_vec)
    ...
    fake_nn = models.feed_forward_network(headline_length=MAX_HEADLINE_LENGTH, body_length=MAX_BODY_LENGTH,
